...
function addAppToWorkspaceFile(options: ApplicationOptions, workspace: WorkspaceSchema): Rule {
  ...
  if (options.inlineTemplate === true
    ...
    || options.style !== 'css') {
    schematics['@crexi-dev/schematics:component'] = {};
    if (options.inlineTemplate === true) {
      (schematics['@crexi-dev/schematics:component'] as JsonObject).inlineTemplate = true;
    }
    if (options.inlineStyle === true) {
      (schematics['@crexi-dev/schematics:component'] as JsonObject).inlineStyle = true;
    }
    if (options.style && options.style !== 'css') {
      (schematics['@crexi-dev/schematics:component'] as JsonObject).styleext = options.style;
    }
  }

  if (options.skipTests === true) {
    ['class', 'component', 'directive', 'guard', 'module', 'pipe', 'service'].forEach((type) => {
      if (!(`@crexi-dev/schematics:${type}` in schematics)) {
        schematics[`@crexi-dev/schematics:${type}`] = {};
      }
      (schematics[`@crexi-dev/schematics:${type}`] as JsonObject).spec = false;
    });
  }
